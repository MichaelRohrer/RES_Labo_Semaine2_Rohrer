...
public class BufferedIOBenchmark {
	...
	private String csvLine; //Permet de stocker temporairement une ligne du fichier csv
	private List<String> csvTab = new ArrayList<>(); //Permet de stocker toutes les lignes du fichier csv

	/**Méthode permettant de récupérer dans un fichier au format csv le résultat de lêxécution du programme**/
	public void toCsvData(){
		try{
			OutputStream os = new FileOutputStream("output.csv");
			BufferedOutputStream bos = new BufferedOutputStream(os); //Permet d'améliorer les performances
			PrintStream ps = new PrintStream(bos);

			//Ecriture des lignes dans le fichier
			for(String line : csvTab){
				ps.println(line);
			}
	...
	private void produceTestData(IOStrategy ioStrategy, long numberOfBytesToWrite, int blockSize) {
		...
		//Début de composition de la ligne csv
		csvLine = "WRITE," + ioStrategy.toString() + "," + blockSize + "," + numberOfBytesToWrite + ",";
		...
		csvLine += time; //Fin de composition de la ligne csv
		csvTab.add(csvLine); //Stockage de la ligne dans le conteneur
	}
	...
	private void consumeTestData(IOStrategy ioStrategy, int blockSize) {
		...
		//Début de composition de la ligne csv
		csvLine = "READ," + ioStrategy.toString() + "," + blockSize + ",";
	...
	private void consumeDataFromStream(InputStream is, IOStrategy ioStrategy, int blockSize) throws IOException {
		...
		//Récupération du nombre de bytes lu pour l'intégrer à la ligne csv
		csvLine += totalBytes + ",";
	}
	...
	public static void main(String[] args) {
		...
		//Crée un fichier csv comportant les résultats
		bm.toCsvData();
	}
